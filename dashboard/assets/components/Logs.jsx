...
export const inserter = (limit: number) => (
  update: LogsMessage,
  prev: LogsType
) => {
...
class Logs extends Component<Props, State> {
  ...
  atTop = () =>
    ...
    this.props.container.scrollHeight * requestBand;
  ...
  didUpdate = (prevProps, prevState, snapshot) => {
    if (
      typeof this.props.shouldUpdate.logs === "undefined" ||
    ...
    return (
      ...
      >
        <div style={styles.waitMsg}>
          {this.props.content.logs.endTop
            ? "No more logs."
            : "Waiting for server..."}
        </div>
        <List>
        ...
        {this.props.content.logs.endBottom || (
          <div style={styles.waitMsg}>Waiting for server...</div>
        )}
      </div>
    );
  }
