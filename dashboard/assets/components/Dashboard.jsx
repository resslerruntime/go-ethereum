...
const shouldUpdate = (updater: Object, msg: Object) => {
  ...
  Object.keys(msg).forEach(key => {
    su[key] =
      typeof updater[key] !== "function"
        ? shouldUpdate(updater[key], msg[key])
        : true;
  });
...
const appender = <T>(limit: number, mapper = replacer) => (
  update: Array<T>,
  prev: Array<T>
) => [...prev, ...update.map(sample => mapper(sample))].slice(-limit);
...
class Dashboard extends Component<Props, State> {
  ...
  reconnect = () => {
    ...
    const server = new WebSocket(
      `${window.location.protocol === "https:" ? "wss://" : "ws://"}${
        PROD ? window.location.host : "localhost:8080"
      }/api`
    );
    server.onopen = () => {
  ...
  changeContent = (newActive: string) => {
    this.setState(
      prevState => (prevState.active !== newActive ? { active: newActive } : {})
    );
  };
  ...
  render() {
    return (
      <div className={this.props.classes.dashboard} style={styles.dashboard}>
        <Header switchSideBar={this.switchSideBar} />
        <Body
