          ...
          <ChartRow>
            {this.doubleChart(
              ...
              CPU,
              {
                ...
                tooltip: percentPlotter("Process load")
              },
              {
                data: system.systemCPU,
                tooltip: percentPlotter("System load", multiplier(-1))
              }
            )}
            {this.doubleChart(
              FOOTER_SYNC_ID,
              MEMORY,
              {
                ...
                tooltip: bytePlotter("Active memory")
              },
              {
                data: system.virtualMemory,
                tooltip: bytePlotter("Virtual memory", multiplier(-1))
              }
            )}
            {this.doubleChart(
              FOOTER_SYNC_ID,
              DISK,
              {
                ...
                tooltip: bytePerSecPlotter("Disk read")
              },
              {
                data: system.diskWrite,
                tooltip: bytePerSecPlotter("Disk write", multiplier(-1))
              }
            )}
            {this.doubleChart(
              FOOTER_SYNC_ID,
              TRAFFIC,
              {
                ...
                tooltip: bytePerSecPlotter("Download")
              },
              {
                data: system.networkEgress,
                tooltip: bytePerSecPlotter("Upload", multiplier(-1))
              }
            )}
          </ChartRow>
        </Grid>
        <Grid item>
          ...
          {general.commit && (
            <Typography type="caption" color="inherit">
              ...
              <a
                href={`https://github.com/ethereum/go-ethereum/commit/${
                  general.commit
                }`}
                target="_blank"
